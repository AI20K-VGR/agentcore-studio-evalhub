...
| **L5** | `TraceEvent.tokens` (`prompt` · `completion`) | Bộ chấm **không** đọc — nhưng DoD của chính `#100` viết *"timeline+tokens+cost+citations"* | `render_timeline` in `cost` mà **không** in `tokens`. Nêu ở đây vì nó là ô DoD của DE, không phải nhu cầu của AIE-2 |
