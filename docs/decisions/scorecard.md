...
| **DEC-D28-04** | Khoá dedup lúc hợp nhất golden set là bộ ba **`(tenant, query đã chuẩn hoá, tập `section_roles`)`**, **KHÔNG** phải `query`, cũng không phải `case_id` | `case_id` không dùng được: hai nguồn (`golden_from_kb` sinh máy, bộ người dùng tự nạp) đặt id độc lập nên cùng một câu hỏi mang hai id, không bao giờ gặp nhau. `query` một mình thì **nguy hiểm**, và đây là chỗ số đo đổi hẳn thiết kế: trên 5 bộ đóng gói sẵn có **22 case dùng chung `query` với một case khác** (7/30 · 7/30 · 5/20 · 2/10 · 1/5) — chúng **không phải bản sao** mà là các case hàng rào T1/T6 (*cùng câu hỏi, hỏi dưới tenant khác hoặc bộ vai hẹp hơn*, đáp án đúng là **bị từ chối**). Dedup theo `query` sẽ gộp mỗi cặp đó làm một, tức **xoá lặng lẽ đúng các case bảo mật** mà bộ golden tồn tại để kiểm, và `success_rate` sau đó vẫn ra một con số trông bình thường. Với khoá đủ: **0** trùng ở cả 5 bộ | [evalhub#49](https://github.com/AI20K-VGR/agentcore-studio-evalhub/pull/49) · `golden_merge.py` · `tests/test_golden_merge.py::test_khoa_khong_gay_trung_tren_bo_golden_that` chạy phép đếm này trên corpus thật qua chính `case_key` (không chép lại khoá), nên rút khoá xuống `query` là **đỏ** | ✅ quyết. `section_roles` vào khoá dạng `frozenset` — thứ tự khai trong YAML không phải thông tin |
| **DEC-D28-05** | Chuẩn hoá `query` dừng ở **`NFC` + gộp khoảng trắng + `casefold`**. Cố ý **không** bỏ dấu câu, **không** bỏ dấu thanh | Hai loại sai không cùng giá: chuẩn hoá **thiếu** thì cùng lắm bỏ sót một lần hợp nhất — thấy được, case còn nguyên hai bản; chuẩn hoá **thừa** thì mất case, im lặng, cùng hạng với dedup theo `query`. `NFC` là **bắt buộc** với corpus tiếng Việt chứ không phải trang trí: cùng chữ *"nghỉ"* lưu dạng dựng sẵn hay tổ hợp **hiển thị y hệt** mà `==` trả `False`, nên case người viết và case máy sinh cùng một câu sẽ không bao giờ gặp nhau và luật *"human thắng"* im lặng không áp dụng — lỗi không thấy được khi đọc diff | `normalize_query()` · bài `test_NFC_hai_cach_ma_hoa_dau_tieng_viet_gap_nhau` (có assert chống rỗng: NFD phải khác NFC về byte) · bài `test_dau_cau_khac_nhau_VAN_la_hai_case` khoá **chiều ngược**, đỏ khi ai đó chuẩn hoá mạnh tay thêm | ✅ quyết |
| **DEC-D28-06** | Luật `source` chỉ phân xử **một** cặp: `human` thắng `ai`. Mọi cặp khác (`None` với bất kỳ thứ gì, `ai`–`ai`, `human`–`human`) ⇒ **ném `GoldenSetMergeConflict`**, liệt kê **mọi** va chạm trong một lần | Docstring `GoldenCase.source` khai đúng một luật; module tiêu thụ không được bịa nốt phần còn lại. Đặc biệt **không** coi `None` là `human`: 60 case golden hiện có đều `source=None` (*chưa khai nguồn*), diễn giải chúng thành "người viết" là khai hộ nguồn gốc cho cả 60 — đúng thứ mặc định `None` được chọn để tránh (`DEC-D16-03`, `rate=None ≠ 0.0`). Liệt kê đủ thay vì dừng ở va chạm đầu: người sửa cần một lượt, không phải chạy lại 12 lần cho 12 chỗ | Mutant đã gieo: *"bản tới sau thắng"* (`dict.update` trần) giết **8** bài · *"coi `None` là `human`"* giết bài `[None-ai]` · *"dừng ở va chạm đầu"* giết `test_nem_ra_MOI_va_cham…` | ✅ quyết. 🟡 **điều kiện lật**: ngày DE chốt trục nhãn (`DEC-Q5`) hoặc có luật cho cặp `None`, sửa `_thang()` và bỏ cặp đó khỏi danh sách ném |
...
**Nợ thứ hai, cũng khai rõ:** `merge_golden_sets` (D28-04/05/06) hiện **chưa có caller production**.
Nó là thư viện đúng và đã có máy kiểm, nhưng đường nạp (`POST /api/admin/golden-sets`) vẫn **thay**
cả bộ chứ không hợp nhất. Không tự nối trong cùng PR: đổi một route upload từ *thay* sang *hợp nhất*
là đổi hành vi mà người dùng quan sát được, và nó cần một quyết định riêng (upload mặc định là thay
